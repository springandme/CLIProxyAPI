...
func TestCodexExecutorCacheHelper_OpenAIChatCompletions_StablePromptCacheKeyFromAPIKey(t *testing.T) {
	...
	httpReq, _, _, continuity, err := executor.cacheHelper(
		ctx,
...
func TestCodexExecutorCacheHelper_IdentityConfuseRemapsBodyAndHeaders(t *testing.T) {
	...
	httpReq, body, identityState, continuity, err := executor.cacheHelper(ctx, sdktranslator.FromString("openai-response"), url, auth, req, cliproxyexecutor.Options{}, req.Payload, rawJSON)
	if err != nil {
		t.Fatalf("cacheHelper error: %v", err)
	}
