...
func (e *CodexExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	httpReq, upstreamBody, identityState, continuity, err := e.cacheHelper(ctx, from, url, auth, req, opts, originalPayloadSource, body)
	if err != nil {
		return resp, err
	}
	...
	logCodexRequestDiagnostics(ctx, auth, req, opts, httpReq.Header, upstreamBody, continuity)
...
func (e *CodexExecutor) cacheHelper(ctx context.Context, from sdktranslator.Format, url string, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, userPayload []byte, rawJSON []byte) (*http.Request, []byte, codexIdentityConfuseState, codexContinuity, error) {
	var cache helps.CodexCache
	continuity := codexContinuity{}
	if sourceFormatEqual(from, sdktranslator.FormatClaude) {
		cached, ok, errCache := helps.ClaudeCodePromptCache(ctx, req.Model, req.Payload, nil)
		if errCache != nil {
			return nil, nil, codexIdentityConfuseState{}, codexContinuity{}, errCache
		}
		if ok {
			cache = cached
			continuity = codexContinuity{Key: cache.ID, Source: "claude_prompt_cache"}
		}
		...
		if cache.ID == "" {
			if apiKey := strings.TrimSpace(helps.APIKeyFromContext(ctx)); apiKey != "" {
				...
				continuity = codexContinuity{Key: cache.ID, Source: "client_principal"}
			}
		}
	}
	...
	if identityState.promptCacheKey != "" {
		...
		if continuity.Source == "" {
			continuity.Source = "identity_confuse"
		}
		continuity.Key = identityState.promptCacheKey
	}
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost, url, bytes.NewReader(rawJSON))
	if err != nil {
		return nil, nil, codexIdentityConfuseState{}, codexContinuity{}, err
	}
	if cache.ID != "" {
		httpReq.Header.Set("Session_id", cache.ID)
	}
	applyCodexContinuityHeaders(httpReq.Header, continuity)
	return httpReq, rawJSON, identityState, continuity, nil
}
