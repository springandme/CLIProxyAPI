...
type Service struct {
	store       Repository
	config      func() ManagerCodexInspectionConfig
	authDir     func() string
	authManager func() *coreauth.Manager
	client      *http.Client
...
func New(st Repository, configProvider func() ManagerCodexInspectionConfig, authDirProvider func() string, authManagerProvider func() *coreauth.Manager, clients ...*http.Client) *Service {
...
func (s *Service) Run(ctx context.Context, req RunRequest) (RunDetail, error) {
	...
	settings, err := s.resolveRuntime(ctx)
	if err != nil {
		return RunDetail{}, err
	}
	...
	run, err := s.store.CreateCodexInspectionRun(ctx, CodexInspectionRun{
		TriggerType:  triggerType,
		TriggerKey:   strings.TrimSpace(req.TriggerKey),
		Status:       CodexInspectionStatusRunning,
		StartedAtMS:  startedAt,
		Settings:     settings,
		SettingsJSON: MarshalCodexInspectionSettings(settings),
	})
	if err != nil {
		return RunDetail{}, err
	}
	...
	files, err := s.fetchAuthFiles(ctx)
	if err != nil {
	...
	results := s.inspectAccounts(ctx, settings, run.ID, sampled, logger)
	if err := ctx.Err(); err != nil {
		...
		run.Status = CodexInspectionStatusFailed
		run.Error = err.Error()
	...
	actionOutcomes := s.executeAutoActions(ctx, settings, results, logger)
...
func (s *Service) ExecuteManualActions(ctx context.Context, runID int64, req ExecuteActionsRequest) (ExecuteActionsResult, error) {
	...
	if detail.Run.Status != CodexInspectionStatusCompleted {
	...
	validItems, validationOutcomes, err := s.validateManualActionItems(ctx, persistCtx, items, logger)
	if err != nil {
		return ExecuteActionsResult{}, err
	}
	...
	outcomes = append(outcomes, s.executeActionItems(ctx, settings, validItems, logger, "手动处理", func(item CodexInspectionResult) string {
		return item.Action
	})...)
	if len(outcomes) == 0 {
		return ExecuteActionsResult{}, ErrNoActionableResults
...
func (s *Service) ResolveConfig(ctx context.Context) (ManagerCodexInspectionConfig, bool, error) {
	_ = ctx
	if s.config == nil {
		return DefaultCodexInspectionConfig(), false, nil
	}
	return NormalizeCodexInspectionConfig(
		s.config(),
...
func (s *Service) resolveRuntime(ctx context.Context) (ManagerCodexInspectionConfig, error) {
	_ = ctx
	if s.config == nil || s.currentAuthManager() == nil {
		return ManagerCodexInspectionConfig{}, ErrNotConfigured
	}
	settings := NormalizeCodexInspectionConfig(
		s.config(),
		DefaultCodexInspectionConfig(),
	)
	return settings, nil
...
func (s *Service) failRun(ctx context.Context, run CodexInspectionRun, cause error) (RunDetail, error) {
	run.Status = CodexInspectionStatusFailed
...
func (s *Service) fetchAuthFiles(ctx context.Context) ([]authFile, error) {
	_ = ctx
	...
	auths := manager.List()
	files := make([]authFile, 0, len(auths))
	for _, auth := range auths {
		if file := s.authFileFromAuth(auth); file != nil {
...
func (s *Service) authFileFromAuth(auth *coreauth.Auth) authFile {
	if auth == nil {
		return nil
	}
	auth.EnsureIndex()
	...
	file := authFile{
		...
		"metadata":       mapAnyCopy(auth.Metadata),
		"attributes":     mapStringAnyCopy(auth.Attributes),
	}
	if auth.CreatedAt.IsZero() {
		file["created_at"] = auth.CreatedAt
	}
	if !auth.UpdatedAt.IsZero() {
		file["updated_at"] = auth.UpdatedAt
	}
	if !auth.LastRefreshedAt.IsZero() {
		file["last_refresh"] = auth.LastRefreshedAt
	}
	...
	if auth.Attributes != nil {
		for _, key := range []string{"account_id", "chatgpt_account_id", "plan_type", "state"} {
...
func mapAnyCopy(in map[string]any) map[string]any {
	if len(in) == 0 {
		return nil
	}
...
func mapStringAnyCopy(in map[string]string) map[string]any {
	if len(in) == 0 {
		return nil
	}
	out := make(map[string]any, len(in))
...
func (s *Service) authByIndex(authIndex string) (*coreauth.Auth, bool) {
	...
	for _, auth := range manager.List() {
		...
		if strings.TrimSpace(auth.EnsureIndex()) == authIndex {
			return auth, true
		}
	}
	return nil, false
...
func (s *Service) inspectAccounts(
	...
	results := make(chan CodexInspectionResult, len(accounts))
	...
	for i := 0; i < workers && i < len(accounts); i++ {
		...
		go func() {
			defer wg.Done()
			for item := range jobs {
				result := s.inspectSingleAccount(ctx, settings, item, logger)
				result.RunID = runID
	...
	out := make([]CodexInspectionResult, 0, len(accounts))
...
func (s *Service) inspectSingleAccount(
	...
	for attempt := 0; attempt <= settings.Retries; attempt++ {
		response, err = s.requestCodexUsage(ctx, settings, item)
		if err == nil {
			break
		}
	}
...
func (s *Service) requestCodexUsage(
	...
	auth, ok := s.authByIndex(item.AuthIndex)
	if !ok {
		return apiCallResponse{}, fmt.Errorf("auth_index %q not found", item.AuthIndex)
	}
	...
	if strings.TrimSpace(item.AccountID) != "" {
		req.Header.Set("Chatgpt-Account-Id", strings.TrimSpace(item.AccountID))
	}
...
func (s *Service) executeAutoActions(
	...
	mode := NormalizeCodexInspectionAutoActionMode(settings.AutoActionMode, CodexInspectionAutoActionNone)
	...
	outcomes = append(outcomes, preflightOutcomes...)
	outcomes = append(outcomes, s.executeActionItems(ctx, settings, items, logger, "自动处理", func(item CodexInspectionResult) string {
...
func (s *Service) executeActionItems(
	...
	logger runLogger,
	logPrefix string,
	actionFor func(CodexInspectionResult) string,
) []ActionOutcome {
	workers := settings.DeleteWorkers
	if workers <= 0 {
		workers = 1
	}
	jobs := make(chan CodexInspectionResult)
		...
		go func() {
			...
			for {
				...
				case item, ok := <-jobs:
					...
					if err := s.executeAction(ctx, actionItem); err != nil {
						outcome.Success = false
						outcome.Status = CodexInspectionActionStatusFailed
					...
					outcome.Status = CodexInspectionActionStatusSuccess
					outcomes <- outcome
...
func tokenValueFromMetadata(metadata map[string]any) string {
	...
	for _, key := range []string{"token", "id_token", "cookie"} {
		value := metadata[key]
		if text, ok := value.(string); ok {
			if token := strings.TrimSpace(text); token != "" {
				return token
			}
		}
		if record, ok := value.(map[string]any); ok {
			if text, ok := record["access_token"].(string); ok {
				if token := strings.TrimSpace(text); token != "" {
					return token
				}
			}
...
func (s *Service) executeAction(ctx context.Context, item CodexInspectionResult) error {
	manager := s.currentAuthManager()
	if manager == nil {
	...
	switch item.Action {
	case "delete":
		return s.deleteAuthFile(ctx, item.FileName)
	case "disable", "enable":
		disabled := item.Action == "disable"
		return s.setAuthDisabled(ctx, item.FileName, disabled)
	default:
		return nil
	}
}

func (s *Service) setAuthDisabled(ctx context.Context, name string, disabled bool) error {
	manager := s.currentAuthManager()
	if manager == nil {
		return ErrNotConfigured
	}
	auth, ok := s.authByName(name)
	if !ok {
	...
	if disabled {
		...
		auth.StatusMessage = "disabled via codex inspection"
	} else {
...
func (s *Service) authByName(name string) (*coreauth.Auth, bool) {
...
func (l runLogger) log(ctx context.Context, level string, message string, detail any) {
	...
	_, _ = l.service.store.InsertCodexInspectionLog(ctx, CodexInspectionLog{
		RunID:   l.runID,
		Level:   level,
		Message: message,
...
func resolveAutoActionResults(mode string, results []CodexInspectionResult) []CodexInspectionResult {
	mode = NormalizeCodexInspectionAutoActionMode(mode, CodexInspectionAutoActionNone)
	if mode == CodexInspectionAutoActionNone {
		return results
	}
	out := make([]CodexInspectionResult, len(results))
	copy(out, results)
	return out
}

func resolveExecutableAction(mode string, action string) string {
	if mode == CodexInspectionAutoActionDisable && action == "delete" {
		return "disable"
	}
	return action
}

func selectAutoActionItems(mode string, results []CodexInspectionResult) ([]CodexInspectionResult, []ActionOutcome) {
	mode = NormalizeCodexInspectionAutoActionMode(mode, CodexInspectionAutoActionNone)
	if mode == CodexInspectionAutoActionNone {
		return nil, nil
	}

	items := make([]CodexInspectionResult, 0)
...
func buildExecutableFileActionGroups(results []CodexInspectionResult) []fileActionGroup {
...
func allowAutoAction(mode string, result CodexInspectionResult) bool {
	switch mode {
...
func selectManualActionItems(
	results []CodexInspectionResult,
	selected map[int64]struct{},
) ([]CodexInspectionResult, []ActionOutcome) {
	items := make([]CodexInspectionResult, 0, len(selected))
	outcomes := make([]ActionOutcome, 0)
	...
	for _, result := range results {
		...
		switch NormalizeCodexInspectionActionStatus(result.ActionStatus, result.Action) {
...
func (s *Service) validateManualActionItems(
	...
	items []CodexInspectionResult,
	logger runLogger,
) ([]CodexInspectionResult, []ActionOutcome, error) {
	if len(items) == 0 {
		return nil, nil, nil
	}
	files, err := s.fetchAuthFiles(ctx)
	...
	validItems := make([]CodexInspectionResult, 0, len(items))
	outcomes := make([]ActionOutcome, 0)
	for _, item := range items {
...
func matchCurrentAccount(candidates []account, result CodexInspectionResult) (account, bool) {
	if len(candidates) == 0 {
...
func summarizeRun(run CodexInspectionRun, results []CodexInspectionResult) CodexInspectionRun {
	run.DisabledCount = 0
	run.EnabledCount = 0
	run.DeleteCount = 0
	run.DisableCount = 0
...
func applyActionOutcomes(results []CodexInspectionResult, outcomes []ActionOutcome) []CodexInspectionResult {
	if len(outcomes) == 0 {
		return results
	}
	...
	for i := range out {
		...
		status := NormalizeCodexInspectionActionStatus(outcome.Status, out[i].Action)
		if status == CodexInspectionActionStatusPending {
			if outcome.Success {
				status = CodexInspectionActionStatusSuccess
			} else {
				status = CodexInspectionActionStatusFailed
			}
		}
		out[i].ActionStatus = status
		out[i].ActionError = outcome.Error
		out[i].ExecutedAction = ""
		if status == CodexInspectionActionStatusSuccess {
...
func failedActionOutcome(item CodexInspectionResult, action string, message string) ActionOutcome {
...
func needsReviewActionOutcome(item CodexInspectionResult, action string, message string) ActionOutcome {
	return ActionOutcome{
		ResultID:       item.ID,
		AccountKey:     item.AccountKey,
		FileName:       item.FileName,
		DisplayAccount: item.DisplayAccount,
		Action:         action,
		Status:         CodexInspectionActionStatusNeedsReview,
		Success:        true,
		Error:          message,
	}
}

func skippedActionOutcome(item CodexInspectionResult, action string, message string) ActionOutcome {
	return ActionOutcome{
		ResultID:       item.ID,
		AccountKey:     item.AccountKey,
		FileName:       item.FileName,
		DisplayAccount: item.DisplayAccount,
		Action:         action,
		Status:         CodexInspectionActionStatusSkipped,
		Success:        true,
		Error:          message,
	}
}
...
func resultFromAccount(item account) CodexInspectionResult {
	return CodexInspectionResult{
		AccountKey:     item.Key,
		FileName:       item.FileName,
		DisplayAccount: item.DisplayAccount,
		AuthIndex:      item.AuthIndex,
...
func buildCodexInspectionQuotaWindows(payload map[string]any, planType string) []CodexInspectionQuotaWindow {
	if payload == nil {
		return nil
	}
	teamPlan := normalizeCodexPlanType(firstNonEmpty(planType, readString(payload, "plan_type", "planType"))) == "team"
	windows := make([]CodexInspectionQuotaWindow, 0)
...
func addCodexRateLimitWindows(
	windows *[]CodexInspectionQuotaWindow,
	limit *codexRateLimit,
...
func addCodexWindowInfo(
	...
	*windows = append(*windows, CodexInspectionQuotaWindow{
		ID:                 id,
		LabelKey:           labelKey,
